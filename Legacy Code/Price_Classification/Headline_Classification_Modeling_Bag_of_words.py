...
dir_ = 'D:/Thesis_Model/Price_Classification'
...
model.add(Embedding(num_w, 56, input_length=max_length))
...
pred_high = pred[test]
pred_high[:] = 1
y_test_high = y_test.reshape(-1, 1)[test].reshape(1, -1)[0]

test = pred < 0.48
pred_low = pred[test]
pred_low[:] = 0
y_test_low = y_test.reshape(-1, 1)[test].reshape(1, -1)[0]

pred_test = np.append(pred_high, pred_low)
y_tester = np.append(y_test_high, y_test_low)

confusion_matrix(y_tester, pred_test)
